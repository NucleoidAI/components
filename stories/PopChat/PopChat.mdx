...
import * as PopChat from "./PopChat.stories.js";

<Meta of={PopChat} />

# PopChat

The `PopChat` component represents a draggable and resizable chat window interface powered by the OpenAI chatbot, NucBot. The component includes a chat header, content area for displaying chat history, a text field for new user messages a button for submitting new messages and allows the use of dictation.

## Usage

To make use of the `PopChat` component, you will need to pass certain props to it. Here's an explanation of each prop:
